...
class tx_tipafriend extends tslib_pibase {
	...
	function tipform()	{
		...
		$markerArray=array();
		$markerArray['###FORM_URL###'] = $this->pi_linkTP_keepPIvars_url(false, false, true);
		$markerArray['###URL###']=$url;
	...
	/**
	 * Check if a given string is valid as tip URL
	 *
	 * @param	string	the URL to be checked
	 * @return	string	the URL if valid, else an empty string
	 */
	function validateUrl($url)	{
			// check for empty string or hmtl tags in url
		if (!$url || ($url != strip_tags($url))) {
			$url = '';
		}
			// If the URL contains a '"', unset $url (suspecting XSS code)
		if ($this->removeXss($url) !== $url) {
			$url = '';
		}
	...
	function validate(&$tipData,$captchaStr='')	{
		...
		$message = $this->removeXss($tipData['message']);
		if ($message != $tipData['message']) {
			$tipData['message'] = $message;
			$ret = false;
		}
...
//		debug($tConf);
	...
	/**
	 * Wrapper function for different versions of the RemoveXSS script
	 * 
	 * @parameter	string	the string to be cleaned
	 * @return	string
